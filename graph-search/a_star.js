...
function draw() {
    if (toVisit.length) {
        ...
        let fScores = toVisit.map(node => node.totalCost);
        let minIndex = toVisit.indexOf(Math.min(...fScores));
        let minNode = toVisit.splice(minIndex, 1)[0];
        ...
        for (let i = 0; i < minNode.connections.length; ++i) {
            ...
            if (!toVisit.includes(connection)) toVisit.push(connection);
