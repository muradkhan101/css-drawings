...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.setInterval(() => {
        ...
        for (let i = canvas.width * 4 * canvas.height - 4; i >= 0 ; i -= pixelWidth) {
            if (isEmpty(imgData.data, getPixelBelow(i))) {
                console.log('Pixel empty');
                copyPixel(imgData.data, getPixelBelow(i), i);
                continue;
            }
            const bottomLeftRightPixels = [getPixelBottomLeft(i), getPixelBottomRight(i)].filter(p => isEmpty(imgData.data, p));
            if (!bottomLeftRightPixels.length) {
                console.log('bot empty');
                continue;
            };
            if (bottomLeftRightPixels.length === 1) {
                console.log('bottom left or right empty');
                copyPixel(imgData.data, bottomLeftRightPixels[0], i);
            } else {
                console.log('bottom left / right empty');
                const pixelNum = Math.floor(Math.random() + 0.5);
                console.log(pixelNum);
                copyPixel(imgData.data, bottomLeftRightPixels[pixelNum], i);
            }
        }
        // const newImageData = new ImageData(imgData, canvas.width, canvas.height);
